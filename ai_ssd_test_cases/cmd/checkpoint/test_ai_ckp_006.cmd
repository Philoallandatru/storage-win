@echo off
REM =============================================================================
REM  AI-CKP-006 :: 1T 子集 save/load（native，破坏性）
REM  model = llama3-1t  |  accelerator = A100  |  data_format = ckpt  |  容量 ~ 11392 GiB
REM  执行方式 = native（原生 mlpstorage 命令）  |  破坏性 = 是（破坏性，必须 --confirm-dut）  |  家族 = checkpoint（检查点）
REM  预期阻塞：破坏性；需要 12 TiB；必须 --confirm-dut
REM
...
REM    ai_ssd_test_cases\cmd\checkpoint\test_ai_ckp_006.cmd
REM ============================================================================
...
"%PY%" -m ai_ssd_test_cases.test_checkpoint_1t_subset ^
    --data-dir "%DUT%" ^
    --result-dir "%RES%" ^
    --execute ^
    --confirm-dut
