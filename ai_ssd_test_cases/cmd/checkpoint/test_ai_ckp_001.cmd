@echo off
REM =============================================================================
REM  AI-CKP-001 :: 8B 全量 baseline save/load（native，破坏性）
REM  model = llama3-8b  |  accelerator = A100  |  data_format = ckpt  |  容量 ~ 840 GiB
REM  执行方式 = native（原生 mlpstorage 命令）  |  破坏性 = 是（破坏性，必须 --confirm-dut）  |  家族 = checkpoint（检查点）
REM  预期阻塞：破坏性；需要 840 GiB；必须 --confirm-dut
REM
...
REM    ai_ssd_test_cases\cmd\checkpoint\test_ai_ckp_001.cmd
REM ============================================================================
...
"%PY%" -m ai_ssd_test_cases.test_checkpoint_8b_full_baseline ^
    --data-dir "%DUT%" ^
    --result-dir "%RES%" ^
    --execute ^
    --confirm-dut
