@echo off
REM ==============================================================================
REM  AI-VDB-002 :: 1M HNSW 内存图 (native)
REM  model = hnsw-1m  |  accelerator = *  |  data_format = vdb  |  capacity ~ 30 GiB
REM  execution = native  |  destructive = False  |  family = vectordb
REM  expected blocker: native: mlpstorage open vectordb hnsw search file (1M 1536D)
REM
...
"%PY%" -m ai_ssd_test_cases.test_vdb_hnsw_1m ^
    --data-dir "%DUT%" ^
    --result-dir "%RES%" ^
    --execute
set RC=%ERRORLEVEL%
