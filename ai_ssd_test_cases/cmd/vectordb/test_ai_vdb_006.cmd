@echo off
REM ==============================================================================
REM  AI-VDB-006 :: 10M HNSW 加载 (native)
REM  model = hnsw-10m  |  accelerator = *  |  data_format = vdb  |  capacity ~ 300 GiB
REM  execution = native  |  destructive = False  |  family = vectordb
REM  expected blocker: native: mlpstorage open vectordb hnsw search file (10M 1536D)
REM
...
"%PY%" -m ai_ssd_test_cases.test_vdb_hnsw_10m ^
    --data-dir "%DUT%" ^
    --result-dir "%RES%" ^
    --execute
set RC=%ERRORLEVEL%
