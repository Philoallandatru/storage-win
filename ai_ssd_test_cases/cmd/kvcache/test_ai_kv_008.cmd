@echo off
REM =============================================================================
REM  AI-KV-008 :: Llama3.1 70B 多用户（native）
REM  model = llama31-70b-users  |  accelerator = A100  |  data_format = kv  |  容量 ~ 1 GiB
REM  执行方式 = native（原生 mlpstorage 命令）  |  破坏性 = 否  |  家族 = KV Cache
REM  预期阻塞：Llama3.1 70B 多用户并发
REM
...
REM    ai_ssd_test_cases\cmd\kvcache\test_ai_kv_008.cmd
REM ============================================================================
...
"%PY%" -m ai_ssd_test_cases.test_kv_llama31_70b_users ^
    --data-dir "%DUT%" ^
    --result-dir "%RES%" ^
    --execute
set RC=%ERRORLEVEL%
