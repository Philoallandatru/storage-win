r"""Wrapper for AI-TRN-014 — Training concurrency, python_scaled, 1.0 GiB dataset.

Original xlsx row: 14 (Training Case表).

xlsx 不合理（已在 wrapper 里修正）:
  xlsx 'workers 1..16 sweep' 在 Python 缩版下不能复现；sweep 实现缺失
...
Usage::
    ...
    .venv\Scripts\python.exe ai_ssd_test_cases\training\test_training_ai_trn_014_training-concurrency.py ^
