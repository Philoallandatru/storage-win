r"""Wrapper for AI-TRN-016 — warm/cold/direct, python_scaled, 1.0 GiB dataset.

Original xlsx row: 16 (Training Case表).

xlsx 不合理（已在 wrapper 里修正）:
  xlsx '三路径对比' 在 Python 缩版下不能复现；--direct 是占位

真实根因（在本工作站上跑之前要知道）:
  verdict=NOT_FORMAL；wrapper 只跑 buffered 路径

Usage::

    .venv\Scripts\python.exe ai_ssd_test_cases\training\test_training_ai_trn_016_warm-cold-direct.py ^
