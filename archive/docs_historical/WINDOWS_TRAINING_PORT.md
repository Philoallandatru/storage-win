...
|---------|------|
| 装环境 | `uv lock && uv sync` |
| 跑 unet3d 端到端 | `mlpstorage closed training unet3d run --exec-type docker --systemname test_nvme --num-accelerators 2 --accelerator-type h100 --client-host-memory-in-gb 64 --data-dir D:\data --results-dir D:\results` |
| 跑 dlio_benchmark 直跑 | `dlio_benchmark workload=unet3d_a100 --data-folder=D:\data\unet3d --output-folder=D:\results` |
| 测存储（不要训练） | `mlpstorage closed training unet3d datasize --data-dir D:\data` |
| 看环境哪些行被改过 | `git diff --stat` |
| 一键回滚 | `git checkout -- pyproject.toml mlpstorage_py/` |
...
| 测试计划 PDF | `<项目根>\docs\AI_SSD_TEST_PLAN.pdf` |
| 测试计划 Excel | `<项目根>\docs\AI_SSD_TEST_PLAN.xlsx` |
...
> **最后更新** 2026-08-02 · **适用 mlpstorage v3.0.46**
> **下一步** 跑通后建议把这份手册和 `AI_SSD_TEST_PLAN.pdf` 一起发给团队评审，再决定是否要 cherry-pick 这 5 个改动到主分支。
