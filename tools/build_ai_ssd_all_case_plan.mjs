...
const caseNumberOffsets = { TRN: 0, BASE: 16, CKP: 20, KV: 29, VDB: 51, MIX: 67 };

function caseNumber(caseId) {
  const [, family, number] = caseId.split("-");
  return caseNumberOffsets[family] + Number(number);
}

function normalizeRow(row) {
  return [null, ...row, ...Array(Math.max(0, 10 - row.length - 1)).fill(null)].slice(0, 10);
}
...
function transformValues(sourceValues) {
  return sourceValues.map((sourceRow, index) => {
    ...
    if (index === 6) {
      row[0] = "序号";
      row[9] = "Capacity";
    } else if (index >= 7 && typeof row[2] === "string" && row[2].startsWith("AI-")) {
      row[0] = caseNumber(row[2]);
      row[3] = "mlperform";
      row[9] = selectedCapacity.get(row[2]) ?? defaultCapacity;
    }
    return row;
  });
}
...
function applyCommonLayout(sheet, name, values) {
  ...
  if (headers.length !== 10 || headers[0] !== "序号" || headers[1] !== "类别" || headers[9] !== "Capacity") throw new Error(`${name} does not have the expected ten-column header`);
  sheet.showGridLines = false;
  sheet.getRange(`A1:J${endRow}`).values = values;
  setTitle(sheet, "A1:J1", values[0][1]);
  setNote(sheet, "A2:J2", values[1][1]);
  setNote(sheet, "A3:J3", values[2][1]);
  sheet.getRange("B6").formulas = [[`=COUNTA('${name}'!$C$8:$C$${endRow})`]];
  sheet.getRange("A5:J5").format = { fill: colors.blueLight, font: { bold: true, color: colors.navy }, horizontalAlignment: "center", verticalAlignment: "center", wrapText: true };
  sheet.getRange("A6:J6").format = { fill: colors.white, font: { bold: true, color: colors.blue, size: 11 }, horizontalAlignment: "center", verticalAlignment: "center", wrapText: true };
  sheet.getRange("A5:J6").format.borders = { preset: "outside", style: "thin", color: colors.border };
  formatHeader(sheet, "A7:J7");
  formatBody(sheet, `A8:J${endRow}`);
  for (let row = 8; row <= endRow; row++) {
    const caseId = values[row - 1]?.[2];
    sheet.getRange(`A${row}:J${row}`).format.fill = selectedCases.has(caseId) ? colors.green : colors.white;
  }
  sheet.getRange(`A8:A${endRow}`).format = { font: { bold: true, color: colors.navy }, horizontalAlignment: "center", verticalAlignment: "top" };
  sheet.getRange(`C8:C${endRow}`).format = { font: { bold: true, color: colors.blue }, horizontalAlignment: "center", verticalAlignment: "top" };
  sheet.getRange(`J8:J${endRow}`).format = { font: { bold: true, color: colors.navy }, horizontalAlignment: "center", verticalAlignment: "top", wrapText: true };
  sheet.getRange(`A8:J${endRow}`).format.rowHeight = 150;
  sheet.tables.add(`A7:J${endRow}`, true, `${name.replaceAll(" ", "_")}ExecutionTable`);
  ...
  sheet.getRange("F:F").format.columnWidth = 70;
  sheet.getRange("G:G").format.columnWidth = 18;
  sheet.getRange("H:H").format.columnWidth = 65;
...
function buildSelectedSheet(workbook, selectedRows) {
  ...
  sheet.showGridLines = false;
  setTitle(sheet, "A1:J1", "AI SSD 精选 Case 汇总表");
  setNote(sheet, "A2:J2", "汇总 6 类 workload 的首轮精选 Case；测试工具统一为 mlperform，Capacity 为固定 pSLC（SLC）配置下建议执行的 SSD 容量。\n1TB / 2TB / 4TB：可在三种容量上直接对比；2TB / 4TB：建议优先用于较大工作集或持续混合负载。\n");
  setNote(sheet, "A3:J3", "精选数量：29 个；序号与各分类页和测试脚本的 case_no 保持一致。测试命令沿用各 case 的独立脚本入口。");
  sheet.getRange("A5:J5").values = [["精选 Case 数", null, "容量口径", null, "测试工具", null, "步骤格式", null, "固定 SLC", null]];
  sheet.getRange("A6:J6").values = [[null, null, "1TB / 2TB / 4TB", null, "mlperform", null, "1. 2. 3. 4.", null, "固定 pSLC", null]];
  sheet.getRange("B6").formulas = [[`=COUNTA('${name}'!$C$8:$C$${endRow})`]];
  sheet.getRange("A5:J5").format = { fill: colors.blueLight, font: { bold: true, color: colors.navy }, horizontalAlignment: "center", verticalAlignment: "center", wrapText: true };
  sheet.getRange("A6:J6").format = { fill: colors.white, font: { bold: true, color: colors.blue, size: 11 }, horizontalAlignment: "center", verticalAlignment: "center", wrapText: true };
  sheet.getRange("A5:J6").format.borders = { preset: "outside", style: "thin", color: colors.border };
  sheet.getRange("A7:J7").values = [["序号", "类别", "Case ID", "测试工具", "测试目的", "测试步骤", "测试时长", "测试命令", "测试标准", "Capacity"]];
  formatHeader(sheet, "A7:J7");
  sheet.getRange(`A8:J${endRow}`).values = selectedRows;
  formatBody(sheet, `A8:J${endRow}`);
  sheet.getRange(`A8:J${endRow}`).format.fill = colors.green;
  sheet.getRange(`A8:A${endRow}`).format = { font: { bold: true, color: colors.navy }, horizontalAlignment: "center", verticalAlignment: "top" };
  sheet.getRange(`C8:C${endRow}`).format = { font: { bold: true, color: colors.blue }, horizontalAlignment: "center", verticalAlignment: "top" };
  sheet.getRange(`J8:J${endRow}`).format = { font: { bold: true, color: colors.navy }, horizontalAlignment: "center", verticalAlignment: "top", wrapText: true };
  sheet.getRange(`A8:J${endRow}`).format.rowHeight = 150;
  sheet.tables.add(`A7:J${endRow}`, true, "SelectedCaseExecutionTable");
  sheet.freezePanes.freezeRows(7);
  sheet.freezePanes.freezeColumns(3);
  sheet.getRange("A:A").format.columnWidth = 8;
  sheet.getRange("B:B").format.columnWidth = 18;
...
for (const [sourceWorkbook, name] of sourceSheets) {
  ...
  for (const row of values.slice(7)) {
    if (selectedCases.has(row[2])) selectedRows.push(row);
  }
  const preview = await workbook.render({ sheetName: name, range: `A1:J${values.length}`, scale: 1, format: "png" });
...
const selectedPreview = await workbook.render({ sheetName: "精选 Case汇总表", range: `A1:J${7 + selectedRows.length}`, scale: 1, format: "png" });
...
const tableCheck = await workbook.inspect({ kind: "table", sheetId: "精选 Case汇总表", range: `A1:J${7 + selectedRows.length}`, include: "values,formulas", tableMaxRows: 40, tableMaxCols: 10, tableMaxCellChars: 220 });
