...
if (-not $pythonCommand) {
    throw "Python was not found. Install Python 3.12.x and ensure python.exe is on PATH."
}
$pythonVersion = (& $pythonCommand.Source --version 2>&1 | Out-String).Trim()
if ($pythonVersion -notmatch "Python 3\.12\.") {
    throw "This repository requires Python 3.12.x; detected $pythonVersion"
}
...
try {
    ...
    & $uvCommand.Source sync --all-groups --extra test --extra vectordb
    ...
    foreach ($project in @($kvCacheProject, $vdbProject)) {
        if (Test-Path -LiteralPath (Join-Path $project "pyproject.toml")) {
            ...
            & $uvCommand.Source pip install --python $venvPython --editable $project
