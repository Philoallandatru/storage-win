...
| 家族 | 链路验证模式实测 | 压力模式（`--pressure`） | 1.5h 预算余量 |
|---|---|---|---|
| Training（8 文件 / 压力 1200-20000 文件） | 1m44s ~ 4m46s | 预计 15-30 分钟（官方 epochs × 163-234GB 数据） | 充足 |
| Checkpoint（1 写 1 读 / 压力 3 写 3 读） | ~1-5 分钟 | 预计 10-20 分钟（写 ~340GB，512GB 盘安全上限） | 充足 |
| KV Cache（10 users×10s / 压力 50 users×60s） | 9m44s ~ 10m29s | 预计 10-15 分钟（持续 spill） | 充足 |
| VectorDB（100 向量 / 压力 1M 向量×300s） | ~32s | 实测 17m45s（VDB-004 1M×512） | 充足 |

> 结论：**全部 12 场景的每个 case 均满足"运行时间不长于 1.5h"**；KV 家族是耗时上限（MLPerf v3.0 固定 3 个 option），其余家族都在 5 分钟内（链路验证）或 30 分钟内（压力模式）。
